 ...
 * 牛牛想要知道自己要赢得游戏的概率有多大。 输入描述:
 * 输入包含两行，第一行输入三个整数a1，b1，c1(1≤a1，b1，c1≤13)，表示牛牛亮出的扑克牌。
 * 第二行输入三个整数a2，b2，c2(1≤a2，b2，c2≤13)，表示妞妞所亮出的扑克牌。 输出描述:
 * 输出一个数字x（保留4位小数），表示牛牛获胜的概率。 示例1 输入 3 5 7 2 6 8 输出 0.3905
 * 
 * @author kafka
 *
 */
public class Main7 {
	...
	public static void main(String[] args) {
		...
		int[] pokers = new int[52]; // 所有扑克牌 i/4+1 即为牌面
		int sum1 = 0;
		int sum2 = 0;
		for (int i = 0; i < 3; i++) {
			int j = in.nextInt();
			sum1 += j;
			int poke = 4 * j - 4; // 4(j-1)--4*j-1 即为牌面对应的下标
			while (pokers[poke] != 0)
				poke++;
			pokers[poke] = 1;
